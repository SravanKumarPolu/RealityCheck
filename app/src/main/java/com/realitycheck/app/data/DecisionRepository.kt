...
data class AnalyticsData(
    ...
    fun getTimeBasedTrends(): List<WeeklyTrend> {
        ...
        return groupedByWeek.map { (weekKey, weekDecisions) ->
    ...
    fun getBlindSpots(): List<BlindSpot> {
        ...
        if (lowAccuracyDecisions.isEmpty()) return emptyList()
        ...
        val mostCommonMistakeWords = allLowAccuracyWords
            .toList()
    ...
    fun getTopRegretCategory(): Pair<String, Float>? {
        ...
        val regretByCategory = completed.groupBy { it.category!! }
    ...
    fun getRegretScorePerCategory(): Map<String, Float> {
        ...
        return completed.groupBy { it.category!! }
            .mapValues { (_, decisions) ->
                decisions.mapNotNull { it.getRegretIndex() }.average().toFloat()
            }
    }
    ...
    fun getOverconfidenceByCategory(): String? {
        ...
        val categoryOverconfidence = completed.groupBy { it.category!! }
            .mapValues { (_, decisions) ->
    ...
    fun getUnderestimationPattern(): String? {
        ...
        val underestimations = completed.mapNotNull { decision ->
            val moodDiff = decision.actualMood24h!! - decision.predictedMood24h!!
            if (moodDiff < -2f) { // Actual mood was at least 2 points worse
                decision.category to moodDiff
            } else null
        }
    ...
    fun getCategoryAccuracy(): Map<String, Float> {
        ...
        return completed.groupBy { it.category!! }
            .mapValues { (_, decisions) ->
                decisions.mapNotNull { it.getAccuracy() }.average().toFloat()
            }
    }
